...
# Adapted to python3 by Yifan Wang (yfwang09@stanford.edu) 2019.5.18
...
class log:
    ...
    def __init__(self,*list, verbose=True, remove_duplicate=True):
        self.nvec = 0
        self.names = []
        self.ptr = {}
        self.data = []
        self.verbose = verbose
        self.rmdup = remove_duplicate
        ...
        if len(list) == 1:
            ...
            if len(self.flist) > 1:
                raise TypeError("can only incrementally read one log file")
            self.increment = 1
            self.eof = 0
    ...
    def read_all(self):
        ...
        if self.nvec == 0: raise TypeError("log file has no values")
    ...
    def next(self):
        if not self.increment: raise TypeError("cannot read incrementally")
    ...
    def get(self,*keys):
        if len(keys) == 0:
            raise TypeError( "no log vectors specified")
        ...
        for key in keys:
            if key in self.ptr:
                ...
                if count == 1:
                    ...
                    raise TypeError( "unique log vector %s not found" % key)
        ...
        while not last:
            ...
            if self.style == 1:
                ...
                for section in sections:
                    ...
                    words = word1 + word2
                    self.data.append([float(words[i]) for i in range(len(words))])
            ...
            if self.verbose:
                print ('last timestep:', int(self.data[-1][0]))
            sys.stdout.flush()
