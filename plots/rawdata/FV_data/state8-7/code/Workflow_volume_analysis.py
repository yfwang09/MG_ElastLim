...
if status == '0':
    ...
    Sdata_file = output_dir + 'Sdata_T20K_state8_7_0.16_0.17.dat'
    ...
    VFdata = np.zeros((len(frames), 4), dtype=float)
    for f in range(len(frames)):
        frame = frames[f]
        input_dir = input_direct + 'frame_%d/' % frame
    ...
    VFdata_file = output_dir + 'VFdata_T20K_state8_7_0.16_0.17.dat'
    np.savetxt(VFdata_file, VFdata, fmt='%.4f %.4f %.4f %.4f')
...
#     np.savetxt(Sdata_file, Sdata, fmt='%.4f %.3f')
    ...
    for f in range(len(frames)):
        frame = frames[f]
        input_dir = input_direct + 'frame_%d/' % frame
        keyword = 'T%dK_frame_%d'(temp, frame) % (temp, frame)
    ...
    for i in range(len(steps)):
        ...
        step = steps[i]
        init_data = output_dir + 'coords_atoms_%s_0.dump.gz' % keyword
        data = output_dir + 'coords_atoms_%s_%d.dump.gz' % (keyword, i)
        atoms, boundary, dxyz = loadData(data, R1, R2)
        mises_data, disp_data = AtomicStrain(data, init_data)
        id_satoms = mises_data >= mises_crit
        satoms = atoms[id_satoms]

        volume_data = output_dir + 'coords_volume_%s_%d.dump.gz' % (keyword, i)
        ...
        np.savetxt(hist_dir + 'distri_mises_%s_%d.dat' % (keyword, i),
        ...
        np.savetxt(hist_dir + 'distriY_volume_%s_%d.dat' % (keyword, i),
        ...
        volume_group_file = output_dir + 'volume_group_%s_%d.npz' % (keyword, i)
        ...
        np.savez_compressed(hist_dir + 'distri_volume_bin_total_%s_%d.npz' % (keyword, i), hist=hist_total, bins=bins_total)
        np.savez_compressed(hist_dir + 'distri_volume_bin_0_1_%s_%d.npz' % (keyword, i), hist=hist_1, bins=bins_1)
        np.savez_compressed(hist_dir + 'distri_volume_bin_1_9_%s_%d.npz' % (keyword, i), hist=hist_2, bins=bins_2)
    ...
    ############### Calculate the free volume for the specific region, since the shape of STZ is unregular ##########################
    ...
    for f in range(len(frames)):
        ...
        for m in range(len(steps)):
            ...
            extend_atoms, extend_boundary, extend_dxyz = Enlargebox(atoms, boundary, dxyz, dL)
            data1 = temp_dir + 'coords_region1_%s_15.dump.gz' % (keyword)
            data2 = temp_dir + 'coords_region2_%s_15.dump.gz' % (keyword)
            ...
            for p in range(num_refs):
                ...
                print('activated_vol/stz_size:', activated_vol * 0.008/stz_size)
                ...
                ref_atoms_lammps_file = temp_dir + 'coords_region_%d_%s_%d_%.1f.dump.gz' % (p, keyword, step, max_dr)
                SatomGenerateConfiguration(ref_atoms, boundary, dxyz, ref_mises_data, ref_atoms_lammps_file)

            feature_ref_file = output_dir + 'feature_events_regions_%s_%d.dat' % (keyword, m)
            print('\nSaved feature_ref_file:', feature_ref_file)
            np.savez_compressed(output_dir + 'feature_events_regions_%s_%d.npz' % (keyword, m), feature_ref)
    ...
    for max_dr in max_dr_list:
        ...
        for f in range(len(frames)):
            ...
            for m in range(len(steps)):
                ...
                vols_file = output_dir + 'coords_volume_%s_%d.dump.gz' % (keyword, m)
                vols = loadVolumePosition(vols_file)
                atoms, boundary, dxyz = loadDataMore(data, init_data, R1, R2)
                mises_data, disp_data = AtomicStrain(data, init_data)
                ...
                for dn in range(5):
                    ...
                    density_atoms_lammps_file = output_dir + 'open_vol_max_dr_%s_%.1f_dl_%.1f.dump.gz' % (keyword, max_dr, dl)
                    DensityAtomGenerateConfiguration(atoms, boundary, dxyz, atom_feature, density_atoms_lammps_file)

                    np.savez_compressed(output_dir + 'open_vol_max_dr_%s_%.1f_dl_%.1f.npz' % (keyword, max_dr, dl),
    ...
    for f in range(len(frames)):
        ...
        for m in range(len(steps)):
            ...
            for i in range(9):
                ...
                np.savez_compressed(output_dir + 'open_vol_distri_%s_dl_%.1f.npz' % (keyword, dl),
                                    vol_distri, fmt='%.1f, %d, %.4f, %d, %.4f, %.4f')
                vol_distri_file = output_dir + 'open_vol_distri_%s_dl_%.1f.txt' % (keyword, dl)
    ...
    for f in range(len(frames)):
        ...
        for m in range(len(steps)):
            ...
            voro_index_fraction_refs_file = voro_vol_dir + 'voro_indices_fraction_refs_%s_%d.dat' % (keyword, m)
