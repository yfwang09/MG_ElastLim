...
if status == '0':
    ...
    Sdata_file = output_dir + 'Sdata_T20K_state2_1_0.18_0.19.dat'
    ...
    VFdata_file = output_dir + 'VFdata_T20K_state2_1_0.18_0.19.dat'
    ...
    # 'feature_events_T%dK_%.2f.dat'%(temp, strain) would be obtained.
    # run by: ovitos Workflow_volume_analysis.py -n 10
    print('\n status 13: Calculate the average vol for the whole matrix region.')
    print('frames:', frames)
    tp_0 = time.time()
    ...
    # 'feature_events_T%dK_%.2f.dat'%(temp, strain) would be obtained.
    # run by: ovitos Workflow_volume_analysis.py -n 10
    print('\n status 1010: Calculate the average vol for the whole matrix region.')
    print('frames:', frames)
    tp_0 = time.time()
    ...
    for max_dr in max_dr_list:
        ...
        for f in range(len(frames)):
            ...
            for m in range(len(steps)):
                ...
                ref_mises_data = mises_data_i[index_ref_atoms]
                ref_atoms_lammps_file = temp_dir + 'coords_ref_atoms_%s_%d.dump.gz' % (keyword, m)
                AtomGenerateConfiguration(ref_atoms, boundary, dxyz, ref_mises_data, ref_atoms_lammps_file)
